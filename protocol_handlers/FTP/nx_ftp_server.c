...
UINT            status;
    ...
    ftp_server_ptr -> nx_ftp_server_id = NX_FTP_SERVER_ID;
...
UINT    status;
    ...
    /* Check for invalid input pointers.  */
    if ((ftp_server_ptr == NX_NULL) || (ftp_server_ptr -> nx_ftp_server_id != NX_FTP_SERVER_ID))
        return(NX_PTR_ERROR);
...
ULONG                   block_size;
    ...
    for (i = 0; i < NX_FTP_MAX_CLIENTS; i++)
    {
        ...
        connect_ip4_address = client_req_ptr -> nx_ftp_client_request_control_socket.nx_tcp_socket_connect_ip;
                ...
                if (status == FX_SUCCESS)
                {
                    ...
                    else
                    {
                        ...
                        if (status == NX_SUCCESS)
                        {
                            ...
                            status =  nx_tcp_client_socket_connect(&(client_req_ptr -> nx_ftp_client_request_data_socket),
                                         client_req_ptr -> nx_ftp_client_request_control_socket.nx_tcp_socket_connect_ip,
...
UINT status;
    ...
    status = nx_packet_allocate(pool_ptr, packet_ptr, NX_TCP_PACKET, wait_option);

    NX_PARAMETER_NOT_USED(client_request_ptr);
