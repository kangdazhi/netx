...
/*                                                                        */
/**************************************************************************/
static void _nx_tcp_socket_state_data_trim(NX_PACKET *packet_ptr, ULONG amount)
{
ULONG      bytes_to_keep;
NX_PACKET *work_ptr;
    ...
    NX_PACKET *tmp_ptr;
        ...
        if ((int)(work_ptr -> nx_packet_append_ptr - work_ptr -> nx_packet_prepend_ptr) < (int)bytes_to_keep)
        {
            ...
            bytes_to_keep -= (ULONG)(work_ptr -> nx_packet_append_ptr - work_ptr -> nx_packet_prepend_ptr);
        ...
        if (work_ptr)
        {
            ...
            work_ptr -> nx_packet_tcp_queue_next = (NX_PACKET *)NX_PACKET_ALLOCATED;
...
ULONG      work_length;
    ...
    while (amount)
    {
        ...
        work_length = (ULONG)(work_ptr -> nx_packet_append_ptr - work_ptr -> nx_packet_prepend_ptr);
...
NX_IP *ip_ptr;
    ...
    if (packet_data_length)
    {
        ...
        if (((INT)packet_end_sequence - (INT)socket_ptr -> nx_tcp_socket_rx_sequence - (INT)socket_ptr -> nx_tcp_socket_rx_window_current) > 0)
        {
        ...
        (search_end_sequence == socket_ptr -> nx_tcp_socket_rx_sequence))
    {
        ...
        if (search_ptr)
        {
            ...
            search_ptr -> nx_packet_tcp_queue_next =  packet_ptr;
    ...
    {       /* Out of order insertion with packets on the queue. */
        ...
        while (search_ptr)
        {
                ...
                (((int)(search_end_sequence - packet_end_sequence)) >= 0))
            {
                ...
                /* If the sequence number is to the right of the expected sequence number, force an ACK. */
            ...
            NX_PACKET *tmp_ptr;
                ...
                tmp_ptr = search_ptr -> nx_packet_tcp_queue_next;

                /* Mark the packet as no longer being part of the TCP queue. */
                search_ptr -> nx_packet_tcp_queue_next = (NX_PACKET *)NX_PACKET_ALLOCATED;
             ...
             ***************************************************************************************/
            if ((((int)(packet_begin_sequence - search_begin_sequence)) >= 0) &&
                (((int)(packet_end_sequence - search_begin_sequence)) > 0))
            {
        ...
        else
        {
            ...
            previous_ptr -> nx_packet_tcp_queue_next = packet_ptr;
        }
        ...
        else
        {
            ...
            packet_ptr -> nx_packet_tcp_queue_next = search_ptr;
        }
        ...
        do
        {
            ...
            else if (search_ptr -> nx_packet_queue_next != (NX_PACKET *)NX_PACKET_READY)
            {
    ...
    if ((socket_ptr -> nx_tcp_socket_receive_queue_count - acked_packets) > NX_TCP_MAX_OUT_OF_ORDER_PACKETS)
    {
        ...
        socket_ptr -> nx_tcp_socket_receive_queue_tail -> nx_packet_tcp_queue_next = (NX_PACKET *)NX_PACKET_ALLOCATED;
        ...
        if (socket_ptr -> nx_tcp_socket_receive_queue_count > 1)
        {
            ...
            while (search_ptr -> nx_packet_tcp_queue_next != socket_ptr -> nx_tcp_socket_receive_queue_tail)
            {
                search_ptr = search_ptr -> nx_packet_tcp_queue_next;
            }
            ...
            search_ptr -> nx_packet_tcp_queue_next = (NX_PACKET *)NX_PACKET_ENQUEUED;
        }
        else
        {
    ...
    while (acked_packets && socket_ptr -> nx_tcp_socket_receive_suspension_list)
    {
        ...
        /* Does the queue only have one entry?.  */
        if (socket_ptr -> nx_tcp_socket_receive_queue_count == 1)
        {

            /* Yes, single entry queue. Set both head and tail pointers to NULL.  */
            socket_ptr -> nx_tcp_socket_receive_queue_head =  NX_NULL;
            socket_ptr -> nx_tcp_socket_receive_queue_tail =  NX_NULL;
        }
        else
        {
            ...
            /* Simply update the head pointer of the queue. The tail pointer does
               not need update.  */
            socket_ptr -> nx_tcp_socket_receive_queue_head =  packet_ptr -> nx_packet_tcp_queue_next;
        }
        ...
        *((NX_PACKET **)thread_ptr -> tx_thread_additional_suspend_info) =  packet_ptr;
        ...
        /* Determine if the new size is greater than the default window size.  */
        if (socket_ptr -> nx_tcp_socket_rx_window_current > socket_ptr -> nx_tcp_socket_rx_window_default)
        {

            /* Yes, new receive window size is greater than the socket's default,
               adjust to the default window size.  */
