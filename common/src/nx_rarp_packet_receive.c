...
VOID  *additional_info = NX_NULL;
    ...
    else if ((*(message_ptr + 1) & 0xFFFF) == NX_RARP_OPTION_RESPONSE)
    {
        ...
        if (*(message_ptr + 6) && (packet_ptr -> nx_packet_ip_interface -> nx_interface_ip_address == 0))
        {
            ...
            /* Set the IP address of this network interface to the target
               IP address in the RARP response.  */
            packet_ptr -> nx_packet_ip_interface -> nx_interface_ip_address =  *(message_ptr + 6);

            /* Check if we need to continue periodic RARP requests. */

            /* Set a flag initialized to discontinue RARP. */
            continue_RARP = NX_FALSE;
            ...
            for (i = 0; i < NX_MAX_PHYSICAL_INTERFACES; i++)
            {
                ...
                if (ip_ptr -> nx_ip_interface[i].nx_interface_ip_address == 0)
                {
                    ...
                    /* Set the flag to continue RARP requests. */
                    continue_RARP = NX_TRUE;
                    break;
                }
            }

            /* Do we need to continue with periodic RARP requests? */
            if (continue_RARP == NX_FALSE)
            {
                /* No, Disable the RARP activity now that we have all valid IP addresses.  */
