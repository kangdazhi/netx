...
ULONG                  trace_timestamp;
    ...
    *packet_ptr =   NX_NULL;
    ...
    if (!socket_ptr -> nx_tcp_socket_receive_queue_head)
    {
        ...
        /* Determine if the socket is still in an active state (will allow a receive
           socket operation if there are still more queued receive
            ...
            (socket_ptr -> nx_tcp_socket_state >= NX_TCP_CLOSING))
        {
            ...
            /* No the packet cannot receive more data. */
    ...
    if ((head_packet_ptr) && (head_packet_ptr -> nx_packet_queue_next == ((NX_PACKET *)NX_PACKET_READY)))
    {
        ...
        else
        {
            ...
            /* Simply update the head pointer to the packet after the current. The tail pointer does not
               need update.  */
            socket_ptr -> nx_tcp_socket_receive_queue_head =  head_packet_ptr -> nx_packet_tcp_queue_next;
        }
        ...
        head_packet_ptr -> nx_packet_tcp_queue_next =  (NX_PACKET *)NX_PACKET_ALLOCATED;
    ...
    else if ((wait_option) && (_tx_thread_current_ptr != &(ip_ptr -> nx_ip_thread)))
    {
        ...
        {
            NX_TRACE_EVENT_UPDATE(trace_event, trace_timestamp, NX_TRACE_TCP_SOCKET_RECEIVE, 0, *packet_ptr, 0, socket_ptr -> nx_tcp_socket_rx_sequence)
        }
