...
UINT       status;
    ...
    if ((packet_ptr == NX_NULL) || (packet_ptr -> nx_packet_tcp_queue_next != ((NX_PACKET *)NX_PACKET_ALLOCATED)))
    {
    ...
    if ((packet_ptr -> nx_packet_prepend_ptr - (sizeof(NX_IP_HEADER) + sizeof(NX_TCP_HEADER))) < packet_ptr -> nx_packet_data_start)
    {
