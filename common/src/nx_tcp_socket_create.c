...
NX_TCP_SOCKET *tail_ptr;
    ...
    /* Initialize the socket MSS to 0.  The MSS size is filled in when the underlying interface can be
       determined.  For client socket, the MSS is determined inside the nx_tcp_client_connect call.  For
       server sockets, the MSS is determined when the first SYN is received. */
    socket_ptr -> nx_tcp_socket_mss =  0;
    socket_ptr -> nx_tcp_socket_connect_mss =  0;
