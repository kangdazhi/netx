...
UINT status;
    ...
    if ((nx_interface_index >= NX_MAX_IP_INTERFACES)  || (ip_ptr -> nx_ip_interface[nx_interface_index].nx_interface_valid == 0))
    {
    ...
    status =  _nx_igmp_multicast_interface_join(ip_ptr, group_address, nx_interface_index);
