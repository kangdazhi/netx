...
UINT                     index = 0;
    ...
    status =  nx_packet_allocate(dhcp_ptr -> nx_dhcp_packet_pool_ptr, &packet_ptr, NX_UDP_PACKET, NX_DHCP_PACKET_ALLOCATE_TIMEOUT);
...
ULONG           offset;
    ...
    status =  nx_packet_allocate(dhcp_ptr -> nx_dhcp_packet_pool_ptr, &new_packet_ptr, NX_UDP_PACKET, NX_DHCP_PACKET_ALLOCATE_TIMEOUT);
...
#ifdef PACKET_DUMP
{
    ...
    work_ptr = (ULONG *)(packet_ptr -> nx_packet_prepend_ptr - sizeof(NX_UDP_HEADER) - sizeof(NX_IP_HEADER));
    length = ((packet_ptr -> nx_packet_length + sizeof(ULONG) - 1)/sizeof(ULONG)) + sizeof(NX_UDP_HEADER) + sizeof(NX_IP_HEADER);
...
NX_DHCP_CLIENT  *temp_client_rec_ptr;
    ...
    *dhcp_client_ptr = NX_NULL;
    ...
    ip_header_ptr = (NX_IP_HEADER *)(packet_ptr -> nx_packet_prepend_ptr - sizeof(NX_UDP_HEADER) - sizeof(NX_IP_HEADER));
