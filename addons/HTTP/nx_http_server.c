...
UINT    status;
    ...
    if ((server_ptr == NX_NULL) || (server_ptr -> nx_http_server_id != NX_HTTP_SERVER_ID) ||
...
UINT        status;
    ...
    if ((ip_ptr == NX_NULL) || (ip_ptr -> nx_ip_id != NX_IP_ID) ||
        (http_server_ptr == NX_NULL) || (http_server_ptr -> nx_http_server_id == NX_HTTP_SERVER_ID) ||
...
UINT        status;
    ...
    http_server_ptr -> nx_http_server_id =  NX_HTTP_SERVER_ID;
...
UINT    status;
    ...
    /* Check for invalid input pointers.  */
    if ((http_server_ptr == NX_NULL) || (http_server_ptr -> nx_http_server_id != NX_HTTP_SERVER_ID))
        return(NX_PTR_ERROR);
...
UINT    status;
    ...
    /* Check for invalid input pointers.  */
    if ((server_ptr == NX_NULL) || (server_ptr -> nx_http_server_id != NX_HTTP_SERVER_ID) || (header == NX_NULL))
        return(NX_PTR_ERROR);
        ...
        server_ptr -> nx_http_server_authentication_check_extended)
    {
        ...
        if ((status != NX_HTTP_DONT_AUTHENTICATE) && (status != NX_SUCCESS))
        {
            ...
            ULONG client_port;
                ...
                status =   nx_tcp_socket_peer_info_get(&(server_ptr->nx_http_server_socket), &client_address , &client_port);

                if (status == NX_SUCCESS)
                {
                    /* Send this information to the host application. */
                    (server_ptr -> nx_http_server_invalid_username_password_callback)(server_ptr -> nx_http_server_request_resource, client_address, request_type);
                }
            }
        ...
        server_ptr -> nx_http_server_authentication_check_extended)
    {
        ...
        if ((status != NX_HTTP_DONT_AUTHENTICATE) && (status != NX_SUCCESS))
        {
            ...
            ULONG client_port;
                ...
                {
                    /* Send this information to the host application. */
                    (server_ptr -> nx_http_server_invalid_username_password_callback)(server_ptr -> nx_http_server_request_resource, client_address, NX_HTTP_SERVER_PUT_REQUEST);
                }
            }
        ...
        server_ptr -> nx_http_server_authentication_check_extended)
    {
        ...
        if ((status != NX_HTTP_DONT_AUTHENTICATE) && (status != NX_SUCCESS))
        {
            ...
            ULONG client_port;
                ...
                {
                    /* Send this information to the host application. */
                    (server_ptr -> nx_http_server_invalid_username_password_callback)(server_ptr -> nx_http_server_request_resource, client_address, NX_HTTP_SERVER_DELETE_REQUEST);
                }
            }
...
UINT  _nxe_http_server_type_get(NX_HTTP_SERVER *server_ptr, CHAR *name, CHAR *http_type_string)
{
    ...
    /* Check for invalid input pointers.  */
    if ((server_ptr == NX_NULL) || (server_ptr -> nx_http_server_id != NX_HTTP_SERVER_ID) || (name == NX_NULL) || (http_type_string == NX_NULL))
        return(0);
...
UINT  _nxe_http_server_callback_packet_send(NX_HTTP_SERVER *server_ptr, NX_PACKET *packet_ptr)
{
    ...
    /* Check for invalid input pointers.  */
    if ((server_ptr == NX_NULL) || (server_ptr -> nx_http_server_id != NX_HTTP_SERVER_ID) || (packet_ptr == NX_NULL))
        return(NX_PTR_ERROR);
...
UINT  _nxe_http_server_gmt_callback_set(NX_HTTP_SERVER *server_ptr, VOID (*gmt_get)(NX_HTTP_SERVER_DATE *))
{
    ...
    /* Check for invalid input pointers.  */
    if ((server_ptr == NX_NULL) || (server_ptr -> nx_http_server_id != NX_HTTP_SERVER_ID) || (gmt_get == NX_NULL))
        return(NX_PTR_ERROR);
...
UINT  _nxe_http_server_cache_info_callback_set(NX_HTTP_SERVER *server_ptr, UINT (*cache_info_get)(CHAR *, UINT *, NX_HTTP_SERVER_DATE *))
{
    ...
    /* Check for invalid input pointers.  */
    if ((server_ptr == NX_NULL) || (server_ptr -> nx_http_server_id != NX_HTTP_SERVER_ID) || (cache_info_get == NX_NULL))
        return(NX_PTR_ERROR);
...
UINT  _nxe_http_server_mime_maps_additional_set(NX_HTTP_SERVER *server_ptr, NX_HTTP_SERVER_MIME_MAP *mime_maps, UINT mime_maps_num)
{
    ...
    /* Check for invalid input pointers.  */
    if ((server_ptr == NX_NULL) || (server_ptr -> nx_http_server_id != NX_HTTP_SERVER_ID) || (mime_maps == NX_NULL))
        return(NX_PTR_ERROR);
