...
UINT        status;
    ...
    if ((ip_ptr == NX_NULL) || (ip_ptr -> nx_ip_id != NX_IP_ID) || 
        (client_ptr == NX_NULL) || (client_ptr -> nx_http_client_id == NX_HTTP_CLIENT_ID) || 
...
UINT        status;
    ...
    client_ptr -> nx_http_client_id =  NX_HTTP_CLIENT_ID;
...
UINT    status;
    ...
    /* Check for invalid input pointers.  */
    if ((client_ptr == NX_NULL) || (client_ptr -> nx_http_client_id != NX_HTTP_CLIENT_ID))
        return(NX_PTR_ERROR);
...
UINT    status; 
    ...
    /* Check for invalid input pointers.  */
    if ((client_ptr == NX_NULL) || (client_ptr -> nx_http_client_id != NX_HTTP_CLIENT_ID) || (resource == NX_NULL))
        return(NX_PTR_ERROR);
...
/*                                                                        */
/**************************************************************************/
UINT  _nx_http_client_get_start(NX_HTTP_CLIENT *client_ptr, ULONG server_ip, CHAR *resource, CHAR *input_ptr, 
                                UINT input_size, CHAR *username, CHAR *password, ULONG wait_option)
{
...
UINT status;
    ...
    status = _nx_http_client_get_start_extended(client_ptr, server_ip, resource, resource_length, 
                                                input_ptr, input_size, username, username_length,
                                                password, password_length, wait_option);
...
/*    nx_tcp_client_socket_connect          Connect to HTTP Server        */ 
...
UINT        temp_password_length = 0;
    ...
    status = nx_tcp_client_socket_connect(&(client_ptr -> nx_http_client_socket), server_ip, 
...
/*  CALLS                                                                 */ 
/*                                                                        */ 
/*    _nx_http_client_put_start_extended    Actual client put start call  */ 
/*    _nx_utility_string_length_check       Check string length           */ 
/*                                                                        */ 
/*  CALLED BY                                                             */ 
/*                                                                        */ 
...
/*                                                                        */
/**************************************************************************/
UINT  _nx_http_client_put_start(NX_HTTP_CLIENT *client_ptr, ULONG server_ip, CHAR *resource, 
                                CHAR *username, CHAR *password, ULONG total_bytes, ULONG wait_option)
...
UINT status;
    ...
    status = _nx_http_client_put_start_extended(client_ptr, server_ip, resource, resource_length, 
                                                username, username_length, password, password_length, 
                                                total_bytes, wait_option);
